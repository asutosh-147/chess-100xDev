...
import PromotionOptions from './chess-board/PromotionOptions';
...
export const ChessBoard = memo(
  ({
    ...
    const [pawnOptions, setPawnOptions] = useState<React.ReactNode | null>(
      null,
    );
    ...
    const getSelectedPiece = () => {
      return new Promise<string>((resolve) => {
        ...
        setPawnOptions(
          <PromotionOptions onSelect={promote} myColor={myColor} />,
        );
      });
    };
                    ...
                    (square, j) => {
                      ...
                      return (
                        <div
                          ...
                          key={j}
                          className={`${isRightClickedSquare ? (isMainBoxColor ? 'bg-[#CF664E]' : 'bg-[#E87764]') : isKingInCheckSquare ? 'bg-[#FF6347]' : isHighlightedSquare ? `${isMainBoxColor ? 'bg-[#BBCB45]' : 'bg-[#F4F687]'}` : isMainBoxColor ? 'bg-[#739552]' : 'bg-[#EBEDD0]'} ${''} relative`}
                          onContextMenu={(e) => {
                            e.preventDefault();
                          }}
                          onMouseDown={(e) => {
                        ...
                        >
                          {from === squareRepresentation && pawnOptions}
